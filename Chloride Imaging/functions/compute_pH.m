...
%
% then pH is computed from delta and epsilon with a function obtained with
% calibrations in cells.
%
% INPUT:
...
% -delta        coefficient of G1 for each ROI
% -epsilon      coefficient of G2 for each ROI
% -theta        atand(epsilon/delta) for each ROI
% -res          sum of the squared residues between g and delta*G1 + epsilon*G2
% -R2_adj       AVERAGE of the squared residues. Using this metric insead
%               of res, we can compare different fits with different wavelengths. 
...
R2_adj = mean( abs((g - pHmodel)./pHmodel), 2);
