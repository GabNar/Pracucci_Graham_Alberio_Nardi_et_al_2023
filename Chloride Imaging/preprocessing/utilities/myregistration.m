...
for i = 1:nframes
    ...
    %     spacelag = (0:size(c,1))-(size(c,1)-1)/2;
    %                         figure
    %                         imagesc(spacelag,spacelag,c)
    %                         colormap(gray)
    %                         hold on
    %                         plot(xlim,[0 0],'r')
    %                         plot([0 0],ylim,'r')
    [~,m] = max(c,[],'all','linear');
    [deltay,deltax]=ind2sub(size(c),m);
    deltas(1,i) = deltay-(size(c,1)-1)/2;
    deltas(2,i) = deltax-(size(c,2)-1)/2;
